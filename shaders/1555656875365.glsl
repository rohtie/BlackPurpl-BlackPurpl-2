...
float map(vec3 p) {
    ...
    float py = floor(p.y / rep.y);
    float pz = floor(p.z / rep.z);

    // p.y += sin(p.x * 0.2 + time * 2.);
    ...
    float bevel = texture(channel0, p.xz * vec2(1., 2.)).r * 0.25;

    p.y += sin(p.x * 0.65 - localTime * 0.9);
    // p.y += sin(p.x * 0.65 + 1.);
    ...
    vec3 end = vec3(-0.25, 0. + sin(localTime * 0.9), 1.);
...
vec4 pixel(vec2 p) {
    ...
    for (int i=0; i<25; i++) {
        ...
        if (tmp < 0.0001) {
            ...
            return vec4(abs(p.y) * .05, 0.2, 0.3 + p.x * .01, 0.);
            return (
                texture(channel0, p.xz * vec2(1., 2.) - time * 0.2)
    ...
    if (r < 0.5) {
        q.x *= 0.99;
        return texture(channel0, q).gbrr * 1.01 + vec4(p.x * 2., p.y * .05, 0.01, 0.).bgrr;
    }

    return texture(channel0, p).bgrr * 30.;
